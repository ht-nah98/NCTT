import pandas as pd, numpy as np, html as _html
...
if __name__ == "__main__":
    ...
    for name, fn in sorted(list(globals().items())):
        if name.startswith("test_") and callable(fn):
            fn(); ok += 1; print(f"  ✓ {name}")
    print(f"\n{ok} test đơn vị PASS")

    # ── kiểm trên DỮ LIỆU THẬT ──
    import pathlib, json
    N = pathlib.Path("../../niches/christian-blues")
    P = N/"00_input/processed"
    vd = pd.read_parquet(P/"videos.parquet")
    ch = pd.read_parquet(P/"channels.parquet")
    cm = pd.read_parquet(P/"comments.parquet")
    st = pd.read_parquet(P/"video_stats.parquet")
    if "snapshot_at" not in st.columns:
        st = st.assign(snapshot_at=pd.Timestamp("2026-08-13", tz="UTC"))

    crawl = pd.Timestamp("2026-08-13", tz="UTC")
    v = enrich(vd, st, crawl)
    print(f"\n  ✓ enrich: {len(v)} dòng, cột mới = "
          f"{[c for c in ['age_days','vpd','is_matured','outlier_ratio'] if c in v.columns]}")
    print(f"    đã chín: {int(v.is_matured.sum())}/{len(v)} "
          f"({v.is_matured.mean()*100:.1f}%)")

    cm2 = strip_pii(cm)
    assert "author_name" not in cm2.columns
    print(f"  ✓ strip_pii: bỏ author_name, còn {len(cm2.columns)} cột")

    errs = validate(ch, v, st, cm2)
    print(f"  ✓ validate: {len(errs)} lỗi {errs if errs else '(đạt)'}")

    class D:
        videos, comments = v, cm2
        matured = v[v.is_matured]
        meta = {"niche":"christian-blues","crawl_date":"2026-08-13","snapshots":1}
    d = describe_niche(D())
    print(f"  ✓ describe_niche: {d['n_channels']} kênh, "
          f"{d['matured_pct']}% chín, {len(d['caveats'])} caveats")

    f = add_fresh_videos(D(), 200)
    assert len(f.videos) == len(v)+200 and len(f.matured) == len(D.matured)
    print(f"  ✓ add_fresh_videos: +200 video mới, matured KHÔNG đổi")

    print(f"  ✓ format_tool_schemas: "
          f"{len(format_tool_schemas([{'name':'t','description':'d','input_schema':{'properties':{'x':{'type':'string'}}}}]))} ký tự")
    print(f"  ✓ stop_at_gate_1/2: {stop_at_gate_1({})['status']}, {stop_at_gate_2([])['status']}")
    print("\n✅ TOÀN BỘ CODE TRONG 11_CODE_CHAY_DUOC.md CHẠY ĐƯỢC")
